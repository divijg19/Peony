...
REPO="${PEONY_INSTALL_REPO:-divijg19/peony}"
BASE_URL="${PEONY_INSTALL_BASE_URL:-https://github.com/${REPO}/releases/download}"
LATEST_API_URL="${PEONY_INSTALL_LATEST_API_URL:-https://api.github.com/repos/${REPO}/releases/latest}"
...
usage() {
  cat <<'EOF'
Install Peony from a GitHub release.
...
resolve_version() {
  ...
  if [[ -n "${PEONY_INSTALL_LATEST_TAG:-}" ]]; then
    echo "${PEONY_INSTALL_LATEST_TAG}"
    return
  fi
  ...
  if [[ -z "${tag}" ]]; then
    echo "failed to resolve the latest Peony release tag" >&2
    exit 1
  fi
  echo "${tag}"
}
...
asset_name_for() {
  ...
  local os_name="$2"
  local arch="$3"
  echo "peony_${version}_${os_name}_${arch}.tar.gz"
}
...
resolve_rc_file() {
  local shell_name="$1"
  if [[ -n "${PEONY_INSTALL_RC_FILE:-}" ]]; then
    echo "${PEONY_INSTALL_RC_FILE}"
    return
  fi
...
append_bloom_alias_block() {
  local rc_file="$1"
  mkdir -p "$(dirname "${rc_file}")"
  touch "${rc_file}"

  if grep -Fq '# >>> peony bloom alias >>>' "${rc_file}"; then
    return
  fi
...
install_binary() {
  ...
  if command -v install >/dev/null 2>&1; then
    install -m 0755 "${source_bin}" "${BIN_DIR}/${target_name}"
  else
    cp "${source_bin}" "${BIN_DIR}/${target_name}"
    chmod 0755 "${BIN_DIR}/${target_name}"
  fi
}
...
main() {
  ...
  if [[ "${ENABLE_ALIAS}" -eq 1 ]]; then
    ...
    append_bloom_alias_block "${rc_file}"
    echo "Added Bloom alias to ${rc_file}"
  fi
